...
agent_communication:
  ...
  - agent: "main"
    message: "UX improvements: (1) AI Prompts Gallery on song detail now has tap-to-expand. Prompts > 280 chars show a 'Tap to read full prompt' chevron-down hint. Tapping toggles between 6-line preview and full content. (2) Songs catalog now defaults to a compact list view (~56px tall rows with title/artist/status/play/delete) instead of huge cards — much better for managing large catalogs. New header toggle button (grid/list icon) switches between Compact List and Detailed Cards. Preference persisted to AsyncStorage as 'songsViewMode'. Verified visually with both modes rendering correctly. No backend changes."
  - agent: "testing"
